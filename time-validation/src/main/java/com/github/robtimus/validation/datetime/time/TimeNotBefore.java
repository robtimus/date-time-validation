...
import com.github.robtimus.validation.datetime.time.TimeNotBefore.List;
import com.github.robtimus.validation.datetime.time.validators.TimeNotBeforeValidator;

/**
 * Validates that the time part of a date/time object is not before a specific moment in time.
 * More specifically, for a date/time object {@code value}, validates that {@code value.time &gt;= moment}.
 * <p>
 * Supported types are:
 * <ul>
 ...
 */
@Documented
@Constraint(validatedBy = { TimeNotBeforeValidator.ForDate.class,
        TimeNotBeforeValidator.ForCalendar.class,
        TimeNotBeforeValidator.ForInstant.class,
        TimeNotBeforeValidator.ForLocalDateTime.class,
        TimeNotBeforeValidator.ForOffsetDateTime.class,
        TimeNotBeforeValidator.ForZonedDateTime.class
})
@Target({ METHOD, FIELD, ANNOTATION_TYPE, CONSTRUCTOR, PARAMETER, TYPE_USE })
@Retention(RUNTIME)
@Repeatable(List.class)
public @interface TimeNotBefore {

    /**
     * The error message.
     */
    String message() default "{com.github.robtimus.validation.datetime.time.TimeNotBefore.message}";
    ...
    /**
     * Defines several {@link TimeNotBefore} annotations on the same element.
     */
    @Target({ METHOD, FIELD, ANNOTATION_TYPE, CONSTRUCTOR, PARAMETER, TYPE_USE })
    @Retention(RUNTIME)
    @Documented
    public @interface List {
        ...
        /**
         * The {@link TimeNotBefore} annotations.
         */
        TimeNotBefore[] value();
    }
}
